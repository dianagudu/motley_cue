...
from .utils import InfoAuthorisation, MOCK_REQUEST, MOCK_BAD_REQUEST, MOCK_ISS, MOCK_SUB, MOCK_TOKEN
...
async def test_require_authorised_user_bad_request(test_authorisation):
    response = await test_authorisation.authorised_user_required(view_func)(request=MOCK_BAD_REQUEST)
...
async def test_require_authorised_admin_fail(test_authorisation, status_code):
    response = await test_authorisation.authorised_admin_required(view_func)(request=MOCK_REQUEST, iss="anything")
    assert response.status_code == status_code
...
async def test_require_authorised_admin_bad_request(test_authorisation):
    response = await test_authorisation.authorised_admin_required(view_func)(request=MOCK_BAD_REQUEST, iss="anything")
    assert response.status_code == 401
...
async def test_require_authenticated_user_bad_request(test_authorisation):
    response = await test_authorisation.authenticated_user_required(view_func)(request=MOCK_BAD_REQUEST)
    assert response.status_code == 401
